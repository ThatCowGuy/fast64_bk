...
class BK_Import_Settings(PropertyGroup):
...
class BK_Export_Settings(PropertyGroup):
...
bk_import_export_classes = (
    BK_Import_Settings,
    BK_Export_Settings,
)

def f3d_props_register():
    for cls in bk_import_export_classes:
        register_class(cls)


def f3d_props_unregister():
    for cls in reversed(bk_import_export_classes):
