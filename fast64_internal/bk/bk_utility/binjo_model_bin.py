...
class ModelBIN:
        ...
        if (DLSeg.valid == True):
            ...
            for cmd in DLSeg.command_list:
                ...
                if (cmd.command_name == "G_SETTILE"):
                    descriptor_array[active_descriptor].T_clamp     = bool(cmd.parameters[6])
                    descriptor_array[active_descriptor].T_mirror    = bool(cmd.parameters[7])
                    descriptor_array[active_descriptor].T_wrap      = bool(cmd.parameters[8])
                    descriptor_array[active_descriptor].T_shift     = int(cmd.parameters[9])
                    descriptor_array[active_descriptor].S_clamp     = bool(cmd.parameters[10])
                    descriptor_array[active_descriptor].S_mirror    = bool(cmd.parameters[11])
                    descriptor_array[active_descriptor].S_wrap      = bool(cmd.parameters[12])
                    descriptor_array[active_descriptor].S_shift     = int(cmd.parameters[13])
    ...
    def add_and_transform_tri(self, new_tri, tile_descriptor):
        ...
        # apply all the flaggage:
        matching_tri.T_clamp    = tile_descriptor.T_clamp
        matching_tri.T_mirror   = tile_descriptor.T_mirror
        matching_tri.T_wrap     = tile_descriptor.T_wrap
        matching_tri.T_shift    = tile_descriptor.T_shift
        matching_tri.S_clamp    = tile_descriptor.S_clamp
        matching_tri.S_mirror   = tile_descriptor.S_mirror
        matching_tri.S_wrap     = tile_descriptor.S_wrap
        matching_tri.S_shift    = tile_descriptor.S_shift
    ...
    def arrange_mesh_data(self):
        ...
        for tri in self.complete_tri_list:
            ...
            if (tri.visible == True):
                mat = BinjoMaterial(
                    img_alias, coll_encoding,
                    tri.T_clamp, tri.T_mirror, tri.T_wrap, tri.T_shift,
                    tri.S_clamp, tri.S_mirror, tri.S_wrap, tri.S_shift
                )
            else: # (tri.visible == False):
                mat = BinjoMaterial(
                    img_alias, coll_encoding,
                    False, False, 0, 0,
                    False, False, 0, 0
...
class BinjoMaterial:
    def __init__(
        self, img_alias, coll_encoding,
        T_clamp=False, T_mirror=False, T_wrap=0, T_shift=0,
        S_clamp=False, S_mirror=False, S_wrap=0, S_shift=0
    ):
    ...
    def __eq__(self, other):
        if (self.name != other.name):
            return False
        if (
            self.T_clamp  != other.T_clamp  or \
            self.T_clamp  != other.T_clamp  or \
            self.T_mirror != other.T_mirror or \
            self.T_wrap   != other.T_wrap   or \
            self.T_shift  != other.T_shift  or \
            self.S_clamp  != other.S_clamp  or \
            self.S_mirror != other.S_mirror or \
            self.S_wrap   != other.S_wrap   or \
            self.S_shift  != other.S_shift
        ):
            return False
        return True
